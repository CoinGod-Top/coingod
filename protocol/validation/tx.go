...
func (g *GasState) setGas(CGValue int64, txSize int64) error {
	if CGValue < 0 {
		return errors.Wrap(ErrGasCalculate, "input CG is negative")
	}

	g.CGValue = uint64(CGValue)

	var ok bool
	if g.GasLeft, ok = checked.DivInt64(CGValue, consensus.VMGasRate); !ok {
...
func checkValid(vs *validationState, e bc.Entry) (err error) {
	...
	case *bc.Mux:
		...
		for assetID, amount := range parity {
			if assetID == *consensus.CGAssetID {
	...
	case *bc.VoteOutput:
		...
		if *e.Source.Value.AssetId != *consensus.CGAssetID {
	...
	case *bc.Coinbase:
		...
		if *e.WitnessDestination.Value.AssetId != *consensus.CGAssetID {
