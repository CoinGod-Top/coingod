...
type CoingodHook struct {
	logPath string
	lock    *sync.Mutex
}

func newCoingodHook(logPath string) *CoingodHook {
	hook := &CoingodHook{lock: new(sync.Mutex)}
	hook.logPath = logPath
	return hook
}

// Write a log line to an io.Writer.
func (hook *CoingodHook) ioWrite(entry *logrus.Entry) error {
	module := "general"
...
func (hook *CoingodHook) Fire(entry *logrus.Entry) error {
	hook.lock.Lock()
...
func (hook *CoingodHook) Levels() []logrus.Level {
