...
func (a *registerAction) Build(ctx context.Context, b *TemplateBuilder) error {
	...
	if a.AssetId.String() != consensus.CGAssetID.String() {
		return errors.New("register contract action asset must be CG")
	}

	if a.Amount < consensus.BCRPRequiredCGAmount {
		return errors.New("less than BCRP required CG amount")
	}
