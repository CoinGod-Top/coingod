...
import (
	...
	"coingod/net/http/reqid"
)

// Coingod-specific header fields
const (
	HeaderBlockchainID = "Blockchain-ID"
	HeaderCoreID       = "Coingod-Core-ID"
...
// A Client is a Coingod RPC client. It performs RPCs over HTTP using JSON
...
func (c Client) userAgent() string {
	return fmt.Sprintf("Coingod; process=%s; buildtag=%s; blockchainID=%s",
...
func (c *Client) Call(ctx context.Context, path string, request, response interface{}) error {
	...
	if response != nil {
		...
		decoder.UseNumber()
		err = errors.Wrap(decoder.Decode(response))
	}
	return err
}
