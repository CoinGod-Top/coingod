...
func TestReserveCoingodUtxoChain(t *testing.T) {
	...
	for i, c := range cases {
		...
		utxos, err := m.reserveCoingodUtxoChain(&txbuilder.TemplateBuilder{}, "TestAccountID", c.amount, false)
...
func TestBuildCoingodTxChain(t *testing.T) {
	txbuilder.ChainTxUtxoNum = 3
	m := mockAccountManager(t)
	cases := []struct {
		inputUtxo  []uint64
	...
	for caseIndex, c := range cases {
		...
		tpls, gotUtxo, err := m.buildCoingodTxChain(utxos, acct.Signer)
		if err != nil {
			t.Fatal(err)
		}
...
func TestMergeSpendAction(t *testing.T) {
	testCG := &bc.AssetID{}
	if err := testCG.UnmarshalText([]byte("ffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffff")); err != nil {
		t.Fatal(err)
	}
