...
func (m *Manager) reserveCoingodUtxoChain(builder *txbuilder.TemplateBuilder, accountID string, amount uint64, useUnconfirmed bool) ([]*UTXO, error) {
	...
	for gasAmount := uint64(0); reservedAmount < gasAmount+amount; gasAmount = calcMergeGas(len(utxos)) {
		...
		res, err := m.utxoKeeper.Reserve(accountID, consensus.CGAssetID, reserveAmount, useUnconfirmed, nil, builder.MaxTime())
		if err != nil {
			return nil, err
		}
...
func (m *Manager) buildCoingodTxChain(utxos []*UTXO, signer *signers.Signer) ([]*txbuilder.Template, *UTXO, error) {
	if len(utxos) == 0 {
	...
	for index := 0; index < len(utxos); index++ {
		...
		output := types.NewOriginalTxOutput(*consensus.CGAssetID, outAmount, acp.ControlProgram, utxos[index].StateData)
...
func SpendAccountChain(ctx context.Context, builder *txbuilder.TemplateBuilder, action txbuilder.Action) ([]*txbuilder.Template, error) {
	...
	if *act.AssetId != *consensus.CGAssetID {
		return nil, errors.New("spend chain action only support CG")
	}

	utxos, err := act.accounts.reserveCoingodUtxoChain(builder, act.AccountID, act.Amount, act.UseUnconfirmed)
	if err != nil {
		return nil, err
	}
	...
	tpls, utxo, err := act.accounts.buildCoingodTxChain(utxos, acct.Signer)
	if err != nil {
		return nil, err
	}
	...
	if utxo.Amount > act.Amount {
		if err = builder.AddOutput(types.NewOriginalTxOutput(*consensus.CGAssetID, utxo.Amount-act.Amount, utxo.ControlProgram, utxo.StateData)); err != nil {
