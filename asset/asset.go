...
// DefaultNativeAsset native CG asset
...
func initNativeAsset() {
	...
	alias := consensus.CGAlias

	definitionBytes, _ := serializeAssetDef(consensus.CGDefinitionMap)
	DefaultNativeAsset = &Asset{
		Signer:            signer,
		AssetID:           *consensus.CGAssetID,
		Alias:             &alias,
		VMVersion:         1,
		DefinitionMap:     consensus.CGDefinitionMap,
...
// pre-define errors for supporting coingod errorFormatter
var (
	...
	ErrInternalAsset  = errors.New("coingod has been defined as the internal asset")
...
//Asset describe asset on coingod chain
type Asset struct {
	*signers.Signer
...
func (reg *Registry) GetAliasByID(id string) string {
	//coingod
	if id == consensus.CGAssetID.String() {
		return consensus.CGAlias
	}
...
func (reg *Registry) UpdateAssetAlias(id, newAlias string) error {
	...
	if oldAlias == consensus.CGAlias || newAlias == consensus.CGAlias {
